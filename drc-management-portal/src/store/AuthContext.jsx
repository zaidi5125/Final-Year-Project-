...
export function AuthProvider({ children }) {
  ...
  function completePasswordChange(newAccess, newRefresh) {
    if (!user) return
    const updatedUser = { ...user, forcePasswordChange: false }
    localStorage.setItem('drc_access_token', newAccess)
    localStorage.setItem('drc_refresh_token', newRefresh)
    localStorage.setItem('drc_user', JSON.stringify(updatedUser))
    setUser(updatedUser)
    return updatedUser
  }
